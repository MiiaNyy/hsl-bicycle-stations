function formatSeconds (sec) {
	...
	if ( seconds >= 3600 && seconds < 86400 ) { // More than one hour less than one day
		return getSecondsToHMSS( seconds )
	}
	if ( seconds >= 86400 ) { // More than one day
		return getSecondsToDHMSS( seconds )
	} 	else if ( seconds < 3600 ) { // Less than hour
		return ( seconds - ( seconds %= 60 ) ) / 60 + ( 9 < seconds ? ':' : ':0' ) + seconds + " min";
	}
}
